...
import { Plus, ArrowRight, Search, ArchiveX } from "lucide-react";
import { useMemo, useState } from "react";
import { useCategories }
    from "@/app/hooks/catogories/use-categories";
import FormAddTeamCategory from "./form-add-team-cateogory";
...
export default function Categories() {
    ...
    const { response: categories, loading, error, refetch } = useGetData<CategoryResponse[]>(`categories`);
    ...
    return (
        <>
                ...
                <div className="flex-1 bg-(--bg-sidebar) border border-(--border-dark) rounded-xl p-4 lg:h-[calc(100vh-305px)] flex flex-col">
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-4">
                        ...
                        <button
                            ...
                            className="bg-(--btn-activo-sidebar) p-1 rounded hover:scale-105 transition cursor-pointer">
                            <Plus className="w-4 h-4 text-white" />
                        </button>
                    </div>
                            ...
                            <tbody>
                                {
                                    filteredTeams.length === 0 ? (
                                        <tr>
                                            <td
                                                colSpan={4}
                                                className="text-center py-10 text-(--text-btn-sidebar)"
                                            >
                                                <div className="flex flex-col items-center gap-2">
                                                    <ArchiveX className="w-12.5 h-12.5"/>
                                                    <p>No se cuenta con equipos</p>
                                                </div>
                                            </td>
                                        </tr>
                                    ) : (
            ...
            <FormAddTeamCategory isOpen={isOpenForm} setIsOpen={setIsOpenForm} category={activeCategory!} refetch={refetch}></FormAddTeamCategory>
        </>
    );
}